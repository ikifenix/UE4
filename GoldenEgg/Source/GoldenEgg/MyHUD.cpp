...
void AMyHUD::DrawHUD()
{
	...
	DrawText("Greetings from Unreal!", FVector2D(0, 0), hudFont, FVector2D(FontSize, FontSize), FColor::White);
	...
	for (int c = messages.Num() - 1; c >= 0; c--)
	{
		...
		GetTextSize(messages[c].message, outputWidth, outputHeight, hudFont, FontSize);
		float messageH = outputHeight + 2.f*pad;
		float x = 0.f, y = c*messageH;
		// black backing
		DrawRect(FLinearColor::Green, x, y, Canvas->SizeX, messageH);
		// draw our message using the hudFont
		DrawText(messages[c].message, messages[c].color, x + pad, y + pad, hudFont, FontSize);
